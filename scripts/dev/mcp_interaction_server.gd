...
func _ready() -> void:
	# ------------------------------------------------------------------
	# 【本项目加的开关】原版脚本一挂上 autoload 就无条件开 127.0.0.1:9090 并每帧轮询。
	# 这是开发期工具，不该出现在发行的游戏里（正式包既不该开本地端口，
	# 也不该白跑这一套），所以默认【完全不启用】，只在显式传 --mcp 时才打开。
	# 想彻底移除：删掉本文件 + 去掉 project.godot 里的 [autoload] McpInteractionServer 项。
	# ------------------------------------------------------------------
	# 只在【显式传 --mcp】时启用。不按「调试构建就开」是因为：
	# 自检/门禁会反复启动引擎，那样每次都会去占 9090，多实例还会冲突报错。
	# 用法： godot --path <项目> -- --mcp
	# 同时查 args 与 user_args（-- 之后的参数只在 user_args 里）。
	var allow := ("--mcp" in OS.get_cmdline_args()) or ("--mcp" in OS.get_cmdline_user_args())
	if not allow:
		set_process(false)
		return
...
func _exit_tree() -> void:
	var was_running := _server != null or _client != null or _websocket != null
	_clear_debug_draw()
	if _websocket != null:
		_websocket.close()
		_websocket = null
