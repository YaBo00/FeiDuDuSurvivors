...
echo [1/28] syntax gate (CheckAll: compiles every script + scene, ~3s)
...
echo [8/28] shop flow - open / buy / deduct / leave to next wave
"%GC%" --headless --path "%P%" --script res://scripts/dev/ShopFlowProbe.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [9/28] pickup lifetime + env chain
...
echo [10/28] ranged enemy + boss skills - fire / enemy proj hits player / state machine (deterministic)
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeRangedBoss.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [11/28] character traits + weapon table - 6 chars, 176 deterministic assertions
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeCharTraits.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [12/28] floor tile grid - seamless tiling / uniform scale / camera independent / 5 theme textures
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeFloorTiles.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [13/28] endless mode - curve keeps growing past wave 20 / first 20 waves unchanged / no settle at wave 20
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeEndless.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [14/28] endless end-to-end - --selftest --endless, full 40 waves without premature settlement
"%GC%" --headless --path "%P%" "%B%" -- --selftest --endless | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [15/28] darkest-gun end-to-end - --selftest --char sad (pierce 99 / lifesteal / gold_on_hit path)
"%GC%" --headless --path "%P%" "%B%" -- --selftest --char sad | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [16/28] audio - 10 sfx load + BGM is a looping OGG (not a 7MB WAV)
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeAudio.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [17/28] font coverage - subset font must cover every char used in source (anti-tofu)
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeFont.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [18/28] projectile pierce semantics - one projectile hits the same enemy at most once
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbePierce.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [19/28] wave-end upgrade enemy cost - dual vote bundling / level pure / capped / end-to-end
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeUpgradeCost.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [20/28] magnet drop + global magnet - elite guaranteed / timer decay / beyond range / reset
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeMagnet.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [21/28] contact damage sum + cap - swarming stacks up to a bounded cap / iframe single-window kept
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeContactDamage.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [22/28] MetaSave ledger - empty fallback / accumulate / persist / clamp / corrupt-safe
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeMetaSave.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [23/28] spawn tide - pure function bounded / window total exactly conserved
"%GC%" --headless --path "%P%" --script res://scripts/dev/_ProbeSpawnTide.gd | findstr /C:"RESULT=PASS" >nul
if errorlevel 1 (echo   FAIL & set FAIL=1) else (echo   ok)

echo [24/28] MetaSave meta shop - xp conversion / purchase persist / denied when broke or maxed / player bonus applied
