...
echo "[8/29] 商店流程（开店 / 购买 / 扣钱 / 离开进下一波）"
...
# 用退出码判定（quit(0/1)）—— stdout 尾行偶发缓冲截断会让 grep 误报（同 [9][27][28]）
